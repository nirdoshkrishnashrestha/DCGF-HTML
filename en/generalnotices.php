
<?php include("inc/header.php"); ?>

<?php include("inc/nav.php"); ?> 


<section class="container-page">
<div class="container">
<h1>General Notices</h1>
  
<div class="row">
	
<div class="col-lg-9">
<div class="content-page">

	
<div class="notices-page">
	
<table class="table table-bordered align-middle table-striped">
  <thead class="table-dark align-middle">
    <tr>
      <th class="text-center" scope="col">S.N.</th>
      <th scope="col">Title</th>
      <th class="text-center" scope="col">Published Date</th>
    </tr>
  </thead>
  <tbody>
    
	  <tr>
      <th class="text-center" scope="row">4</th>
      <td><a href="download/assistant-level-4.pdf" target="_blank">Assistant Level 4</a></td>
      <td class="text-center">24 Magh, 2077</td>
    </tr>
	 <tr>
      <th class="text-center" scope="row">5</th>
      <td><a href="download/assistant-level-4.pdf" target="_blank">Administrative Manager (CA) Level - 9</a></td>
      <td class="text-center">24 Magh, 2077</td>
    </tr>
	 <tr>
      <th class="text-center" scope="row">6</th>
      <td><a href="download/assistant-level-4.pdf" target="_blank">Notice regarding to Promotion Level 10 (Deputy CEO)</a></td>
      <td class="text-center">24 Magh, 2077</td>
    </tr>
	 <tr>
      <th class="text-center" scope="row">7</th>
      <td><a href="download/assistant-level-4.pdf" target="_blank">Assistant Level 4</a></td>
      <td class="text-center">24 Magh, 2077</td>
    </tr>
	 <tr>
      <th class="text-center" scope="row">8</th>
      <td><a href="download/assistant-level-4.pdf" target="_blank">Administrative Manager (CA) Level - 9</a></td>
      <td class="text-center">24 Magh, 2077</td>
    </tr>
	 <tr>
      <th class="text-center" scope="row">9</th>
      <td><a href="download/assistant-level-4.pdf" target="_blank">Notice regarding to Promotion Level 10 (Deputy CEO)</a></td>
      <td class="text-center">24 Magh, 2077</td>
    </tr>
	 <tr>
      <th class="text-center" scope="row">10</th>
      <td><a href="download/assistant-level-4.pdf" target="_blank">Assistant Level 4</a></td>
      <td class="text-center">24 Magh, 2077</td>
    </tr>
	 <tr>
      <th class="text-center" scope="row">11</th>
      <td><a href="download/assistant-level-4.pdf" target="_blank">Administrative Manager (CA) Level - 9</a></td>
      <td class="text-center">24 Magh, 2077</td>
    </tr>
	 <tr>
      <th class="text-center" scope="row">12</th>
      <td><a href="download/assistant-level-4.pdf" target="_blank">Notice regarding to Promotion Level 10 (Deputy CEO)</a></td>
      <td class="text-center">24 Magh, 2077</td>
    </tr>
	 <tr>
      <th class="text-center" scope="row">13</th>
      <td><a href="download/assistant-level-4.pdf" target="_blank">Assistant Level 4</a></td>
      <td class="text-center">24 Magh, 2077</td>
    </tr>
	 <tr>
      <th class="text-center" scope="row">14</th>
      <td><a href="download/assistant-level-4.pdf" target="_blank">Administrative Manager (CA) Level - 9</a></td>
      <td class="text-center">24 Magh, 2077</td>
    </tr>
	 <tr>
      <th class="text-center" scope="row">15</th>
      <td><a href="download/assistant-level-4.pdf" target="_blank">Notice regarding to Promotion Level 10 (Deputy CEO)</a></td>
      <td class="text-center">24 Magh, 2077</td>
    </tr>
  </tbody>
</table>
	
	<nav aria-label="...">
  <ul class="pagination">
    <li class="page-item disabled">
      <span class="page-link">Previous</span>
    </li>
    <li class="page-item"><a class="page-link" href="#">1</a></li>
    <li class="page-item active" aria-current="page">
      <span class="page-link">2</span>
    </li>
    <li class="page-item"><a class="page-link" href="#">3</a></li>
    <li class="page-item">
      <a class="page-link" href="#">Next</a>
    </li>
  </ul>
</nav>
	
 </div>
	
	
	
</div>
</div>
	

	<?php include("inc/rightpannel.php"); ?>

	
</div>
	
	
</div>  
</section>
  
<?php include("inc/bottom-link.php"); ?>
<?php include("inc/footer.php"); ?> 
